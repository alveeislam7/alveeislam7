# Md Alvee Islam
**BBA (MIS & Marketing) | Data Analyst | Dhaka, Bangladesh**

I bridge the gap between complex data systems and strategic business decisions. I specialize in transforming raw data into actionable insights through robust ETL processes, financial trend modeling, and interactive storytelling.

---

### Technical Expertise
* **Analysis & BI:** Power BI, Tableau, Excel (Advanced), IBM Cognos
* **Data Management:** SQL (MySQL, MS SSMS), Google BigQuery, Python (Pandas, Numpy)
* **Core Skills:** Data Modeling, Business Intelligence, Market Trend Analysis, Financial Data Analysis, ETL Processes

---

### Featured Projects

#### [Bank Loan Portfolio Analysis](https://github.com/alveeislam7/bank-loan-portfolio-analysis) 
*Interactive Power BI Dashboard monitoring financial performance and portfolio risk. Developed custom metrics for growth tracking and asset health for financial stakeholders.*

#### [E-commerce Sales Analysis](https://github.com/alveeislam7/ecommerce-sales-analysis)
*Comprehensive Excel Dashboard analyzing **$2.3M+ in sales**. Automated performance tracking and regional sales distribution metrics.*

---

### Let's Connect
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/md-alvee-islam/)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:alveeislam7@gmail.com)

---
*“Turning data into meaningful impact.”*
